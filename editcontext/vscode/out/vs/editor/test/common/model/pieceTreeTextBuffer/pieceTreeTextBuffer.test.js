...
define(["require", "exports", "assert", "vs/editor/common/controller/wordCharacterClassifier", "vs/editor/common/core/position", "vs/editor/common/core/range", "vs/editor/common/model/pieceTreeTextBuffer/pieceTreeTextBufferBuilder", "vs/editor/common/model/pieceTreeTextBuffer/rbTreeBase", "vs/editor/common/model/textModel", "vs/editor/common/model/textModelSearch"], function (require, exports, assert, wordCharacterClassifier_1, position_1, range_1, pieceTreeTextBufferBuilder_1, rbTreeBase_1, textModel_1, textModelSearch_1) {
    ...
    function createTextBuffer(val, normalizeEOL = true) {
        ...
        return factory.create(1 /* LF */).getPieceTree();
    }
    ...
    suite('snapshot', () => {
        ...
        test('immutable snapshot 1', () => {
            const model = textModel_1.TextModel.createFromString('abc\ndef');
            const snapshot = model.createSnapshot();
            model.applyEdits([
                {
    ...
    suite('chunk based search', () => {
        ...
        test('#45770. FindInNode should not cross node boundary.', () => {
            ...
            assert.deepEqual(ret[0].range, new range_1.Range(2, 3, 2, 4));
            assert.deepEqual(ret[1].range, new range_1.Range(3, 3, 3, 4));
            assert.deepEqual(ret[2].range, new range_1.Range(4, 3, 4, 4));
        });
        test('search searching from the middle', () => {
            ...
            assert.equal(ret.length, 1);
            assert.deepEqual(ret[0].range, new range_1.Range(2, 2, 2, 3));
        });
    });
});
