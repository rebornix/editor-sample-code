...
define(["require", "exports", "assert", "vs/base/common/uri", "vs/platform/markers/common/markerService", "vs/platform/markers/common/markers"], function (require, exports, assert, uri_1, markerService, markers_1) {
    ...
    suite('Marker Service', () => {
        test('query', () => {
            ...
            assert.equal(service.read({ resource: uri_1.URI.parse('file:///c/test/file.cs') }).length, 1);
            assert.equal(service.read({ owner: 'far', resource: uri_1.URI.parse('file:///c/test/file.cs') }).length, 1);
            ...
            assert.equal(service.read({ severities: markers_1.MarkerSeverity.Error }).length, 1);
            assert.equal(service.read({ severities: markers_1.MarkerSeverity.Warning }).length, 1);
            assert.equal(service.read({ severities: markers_1.MarkerSeverity.Hint }).length, 0);
            assert.equal(service.read({ severities: markers_1.MarkerSeverity.Error | markers_1.MarkerSeverity.Warning }).length, 2);
        });
        ...
        test('changeAll must not break integrety, issue #12635', () => {
            ...
            assert.equal(service.read({ owner: 'far' }).length, 2);
            assert.equal(service.read({ resource: uri_1.URI.parse('scheme:path1') }).length, 2);
        });
