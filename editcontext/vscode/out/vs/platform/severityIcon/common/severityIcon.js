 ...
 *--------------------------------------------------------------------------------------------*/
define(["require", "exports", "vs/base/common/severity", "vs/platform/theme/common/themeService", "vs/base/common/color"], function (require, exports, severity_1, themeService_1, color_1) {
    "use strict";
        ...
        function getSVGData(severity, theme) {
            switch (severity) {
                case severity_1.default.Ignore:
                    const ignoreColor = theme.type === themeService_1.LIGHT ? color_1.Color.fromHex('#75BEFF') : color_1.Color.fromHex('#007ACC');
                    return theme.type === themeService_1.LIGHT ? infoStart + encodeURIComponent(ignoreColor.toString()) + infoEnd
                        : infoDarkStart + encodeURIComponent(ignoreColor.toString()) + infoDarkEnd;
                case severity_1.default.Info:
                    const infoColor = theme.type === themeService_1.LIGHT ? color_1.Color.fromHex('#007ACC') : color_1.Color.fromHex('#75BEFF');
                    return theme.type === themeService_1.LIGHT ? infoStart + encodeURIComponent(infoColor.toString()) + infoEnd
                        : infoDarkStart + encodeURIComponent(infoColor.toString()) + infoDarkEnd;
                case severity_1.default.Warning:
                    const warningColor = theme.type === themeService_1.LIGHT ? color_1.Color.fromHex('#DDB100') : color_1.Color.fromHex('#fc0');
                    return theme.type === themeService_1.LIGHT ? warningStart + encodeURIComponent(warningColor.toString()) + warningEnd
                        : warningDarkStart + encodeURIComponent(warningColor.toString()) + warningDarkEnd;
                case severity_1.default.Error:
                    const errorColor = theme.type === themeService_1.LIGHT ? color_1.Color.fromHex('#A1260D') : color_1.Color.fromHex('#F48771');
                    return theme.type === themeService_1.LIGHT ? errorStart + encodeURIComponent(errorColor.toString()) + errorEnd
    ...
    themeService_1.registerThemingParticipant((theme, collector) => {
        collector.addRule(getCSSRule(severity_1.default.Error, theme));
        collector.addRule(getCSSRule(severity_1.default.Warning, theme));
        collector.addRule(getCSSRule(severity_1.default.Info, theme));
        collector.addRule(getCSSRule(severity_1.default.Ignore, theme));
    });
});
